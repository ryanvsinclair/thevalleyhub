...
**Source:** Local pack `/Users/mehdielghissassi/Desktop/clusters/LILLIA/` — `LILLIA .pdf` brochure (18 pp), `LILLIA FLOOR PLAN.pdf` (8 pp), `LILLIA PAYMENT.png`, plus high-res cluster map replacing the pack’s 0.42 MB map: Petkovic broker mirror `Emaar-Lillia-The-Valley-Cluster-Map.pdf` (12.7 MB, embedded map **3308×1436**; saved as `lillia-floorplans/lillia-cluster-map.pdf` / `.jpeg`). Facade exteriors from the same Petkovic listing (Jade/Pearl JPGs). Read 2026-08-13 / 2026-08-14.
...
- **New `plexes` rows (64)** + **`units` rows (406)** — full dataset in [`../../../lillia-floorplans/lillia-units.csv`](../../../lillia-floorplans/lillia-units.csv) / [`lillia-plexes.csv`](../../../lillia-floorplans/lillia-plexes.csv). Sizes **21×4 · 11×6 · 32×8**. Style mix: Jade 10×4 / 5×6 / 17×8 · Pearl 11×4 / 6×6 / 15×8. `street_side` page-relative `up|down|left|right`. Per-unit confidence `unverified`.
...
- **Images in `lillia-floorplans/`** (for later `media` / `media_links`):
...
- **Map provenance:** Pack’s original `LILLIA CLUSTER MAP.pdf` was only 0.42 MB (map raster ~1751×675). Ray authorized replacing with the Petkovic high-res Emaar sheet. High-res lives in `lillia-floorplans/`; overwriting the pack file outside the repo was blocked by the environment — copy `lillia-cluster-map.pdf` over the pack path locally if you want the folders to match.
