...
- **`clusters.amenities` (farm-gardens):** `{'Grand Lawn','Petting Zoo & Animal Farm','Kids Play Area','Hydroponics Greenhouse','Community Farming Allotments','Desert Majlis & Bonfire','Stargazing Platforms','Picnic Spots','Outdoor Fitness Station','Yoga/Events Lawn','Xeriscape Botanical Garden','Events Plaza','Pool Deck','Padel Court','Volleyball Court','Ghaf Forest','Mosque','Wellness Centre','Arrival Plaza'}` — *depends on Doc 4 #05 (`clusters.amenities`), approved 2026-08-08, not yet migrated onto the live schema*
...
- Unit-split and amenities lines above are staged with real values but **cannot be promoted until Doc 4 #05's columns actually exist on the live schema** — approval is recorded, migration execution is a separate, not-yet-authorized step (per #05's Ray's Decision notes).
